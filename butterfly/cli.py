...
@app.command()
def convert(
        input_dir: Path = typer.Argument(..., help="Path to the directory containing source files (e.g., ./input)"),
        output_dir: Path = typer.Argument(..., help="Path to the directory for Markdown output (e.g., ./output)"),
        ...
        dry_run: bool = typer.Option(False, "--dry-run", help="Process files but do not write to disk"),
    ...
    # Initialize pipeline with CLI arguments (Fixes the TypeError)
    ...
    with Progress(
        ...
        for file_path in files:
            ...
            # Call process_file with the correct, updated signature
            ...
            if result.is_non_story:
                ...
                success_count += 1
                if not dry_run and not result.is_non_story:
